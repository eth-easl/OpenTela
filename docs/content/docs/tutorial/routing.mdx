...
3. **Load balancing**: One candidate is selected — by random, round-robin, shortest-queue, or price-weighted policy — unless a `X-Session-Affinity` pin reuses the peer that last served the conversation (see [Session affinity](#session-affinity-sticky-peers-for-kv-cache-warmth)).
...
### Session affinity (sticky peers for KV-cache warmth)

Multi-turn conversations run faster and cheaper when successive turns land
on the **same worker** — its KV (prefix) cache is already warm. A client
opts in by sending one stable header value per conversation:

```python
headers = {"X-Session-Affinity": "conv-4f2a-...", ...}
```

The head keeps an in-memory, TTL'd map of `affinity-key → peer`. On each
request it consults the map **after** narrowing candidates to the
affordable, trusted, ACL-permitted set, and prefers the pinned peer when
it is still a candidate. There is no hard pinning and no server-side
conversation object — the model is best-effort:

- **First turn** (no pin yet): the head selects normally (random /
  round-robin / shortest-queue / price-weighted), then records the peer
  that served.
- **Later turns** (pin present): the head reuses the pinned peer for its
  warm cache.
- **Pinned peer gone / unaffordable / excluded by a retry**: the head
  falls back to its normal policy and re-pins to the peer that *just*
  served. No request is failed by a missing pin.

Because the pin lives in the head's memory and carries a TTL
(`affinity.ttl`, default `10m`, env `OF_AFFINITY_TTL`), stickiness holds
while repeated turns arrive at the same head and within the TTL window.
Disable with `affinity.enabled=false` (env `OF_AFFINITY_ENABLED`). The
same `X-Session-Affinity` value is also mapped to `langfuse.session.id` by
the billing gateway, so the conversation's turns group into one Langfuse
session with zero extra header plumbing.
...
- `X-Session-Affinity` *(optional)* — a stable, client-chosen key for a conversation. The head reuses the peer that last served the same key (sticky sessions / KV-cache warmth), with graceful fallback. See [Session affinity](#session-affinity-sticky-peers-for-kv-cache-warmth).
